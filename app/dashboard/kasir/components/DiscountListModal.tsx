  ...
  useEffect(() => {
    async function loadDiscounts() {
      ...
      try {
        ...
        const mappedCoupons = (couponRes.data || []).map((c: any) => ({
          id: c.id,
          type: 'coupon',
          name: c.name,
          code: c.code,
          description: c.min_purchase > 0 ? `Min. Belanja Rp${c.min_purchase.toLocaleString('id-ID')}` : 'Berlaku untuk semua pembelian',
          discountType: c.type,
          value: c.value,
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[85vh] flex flex-col">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-indigo-50/20 shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-lg">
              <Wallet size={24} />
            </div>
            <div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight">Kupon & Promo Tersedia</h3>
              <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Pilih diskon untuk keranjang ini</p>
            </div>
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-1">
                      <h4 className={`text-sm font-black uppercase tracking-tight ${isApplied ? 'text-white' : (isEligible ? 'text-emerald-700' : 'text-slate-500')}`}>
                  ...
                  <div className={`mt-4 pt-4 border-t ${isApplied ? 'border-white/10' : 'border-slate-200/50'} space-y-3`}>
                    <div className="flex items-center justify-between">
                      <p className={`text-[9px] font-black uppercase tracking-widest ${isApplied ? 'text-indigo-200' : 'text-slate-400'}`}>Keuntungan</p>
                      <p className={`text-xs font-black ${isApplied ? 'text-white' : (isEligible ? 'text-emerald-600' : 'text-slate-500')}`}>
                        {d.discountType === 'percentage' ? `${d.value}% Off` :
                          d.discountType === 'fixed' ? `Discount Rp${d.value.toLocaleString('id-ID')}` :
                            `Gratis ${d.free_product_qty} ${d.free_product?.name || 'Produk'}`}
                      </p>
                    </div>
                  </div>
